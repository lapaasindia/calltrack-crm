...
| **Container (Docker / Coolify)** | `Dockerfile` (client build stage → Node 22 + ffmpeg + tini runtime, `node` user, `CRM_TRUST_PROXY=1` default) behind a TLS-terminating reverse proxy; `docker-compose.yml` for a single box | the `/data` volume (`CRM_DATA_DIR=/data`, `CRM_BACKUP_DIR=/data/backups`: database, session store, `secret.key`, recordings, logs, backups, APK) | Set `CRM_ADMIN_PASSWORD`, `CRM_PUBLIC_URL=https://…` (first in pairing URLs, Drive OAuth own-host) and `CRM_SECURE_COOKIES=true`. Internet-exposed → https only; phones pair with any `https://` host but only private addresses over `http://`. Health check `GET /api/health`; SIGTERM → graceful stop. |
